...
| :--- | :--- |
| `main.py` | Main application entry point; performs dependency checks and initializes queues, logger, hotkey listener, Tkinter GUI, and clean shutdown hooks. |
...
| `window_manager.py` | Manages connection to Waydroid over ADB, verifies authorization status, and streams low-latency screenshots using `adb exec-out screencap -p`. |
...
| `utils.py` | Helper module for subprocess execution, ADB device status checks, and decoding raw screencap bytes into OpenCV BGR NumPy arrays. |
...
## Waydroid + ADB Configuration & Fixing Unauthorized ADB Error
...
2. Connect ADB to Waydroid (default IP is typically `192.168.240.112:5555`):
   ```bash
   adb connect 192.168.240.112:5555
   adb devices
   ```

3. **Fixing ADB `unauthorized` Error**:
   If `adb devices` shows `192.168.240.112:5555 unauthorized`:
   - Restart the ADB server on host:
     ```bash
     adb kill-server
     adb start-server
     adb connect 192.168.240.112:5555
     ```
   - Look inside your Waydroid UI window for the **"Allow USB Debugging?"** Android dialog prompt and check **"Always allow from this computer"** -> **OK**.
   - If no prompt appears, copy your host ADB key directly to Waydroid's authorized keys:
     ```bash
     sudo mkdir -p /var/lib/waydroid/data/misc/adb
     sudo cp ~/.android/adbkey.pub /var/lib/waydroid/data/misc/adb/adb_keys
     sudo waydroid session stop
     waydroid session start
     adb connect 192.168.240.112:5555
     ```

4. Configure custom IP in `config.py` or export environment variables:
   ```bash
...
- **`ModuleNotFoundError: No module named 'cv2'`**:
  You are running `main.py` in a Python environment where dependencies are not installed. Run:
  ```bash
  pip3 install -r requirements.txt
  ```
...
- **ADB Unauthorized / Connection Failed**:
  Follow the **Fixing ADB `unauthorized` Error** section above.
